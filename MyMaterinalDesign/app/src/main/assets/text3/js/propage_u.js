...
$(function(){
		...
		//点击关掉当前层
		$('.shaload span').click(function(e){
				
			$('.shaload').removeClass('currs').css('height','0');
			$('.bainfo').removeClass('disn').addClass('shay');	
	...
	$(window).scroll(function(){
		...
		if($(window).scrollTop()>45){
			...
			$('.shaload').addClass('currs');
		}	
		else{
			$('.bainfo').removeClass('hefixed');	
			$('.shaload').removeClass('currs');
		}
